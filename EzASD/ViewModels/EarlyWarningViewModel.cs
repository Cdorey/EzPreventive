...
namespace EzASD.ViewModels
{
    public class EarlyWarningViewModel : PropertyChangedBase
    {
        ...
        public void LoadRecords(IEnumerable<string> positiveSigns)
        {
            earlyWarningSigns.Where(x => positiveSigns.Contains(x.Question)).ToList().ForEach(x => x.IsPositive = true);
        }

        public IEnumerable<string> SaveRecords()
        {
            return earlyWarningSigns.Where(x => x.IsPositive).Select(x => x.Question);
        }
