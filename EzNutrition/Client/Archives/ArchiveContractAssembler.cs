...
/// <summary>
/// 将 EzNutrition WASM 的运行态咨询快照转换为格式无关档案契约。
/// </summary>
public sealed class ArchiveContractAssembler
{
    ...
    /// <summary>
    /// 建立使用当前 WASM 程序集版本的转换器。
    /// </summary>
    /// <returns>转换器实例。</returns>
    public static ArchiveContractAssembler CreateDefault()
    {
        var version = typeof(ArchiveContractAssembler).Assembly
            .GetCustomAttribute<AssemblyInformationalVersionAttribute>()?
            .InformationalVersion;
        if (string.IsNullOrWhiteSpace(version))
        {
            version = typeof(ArchiveContractAssembler).Assembly.GetName().Version?.ToString() ?? "unknown";
        }
    ...
    private PatientResource CreatePatient(RuntimeArchive archive, DateTimeOffset capturedAt)
    {
    ...
    private EnergyAssessmentResource CreateEnergyAssessment(
        Models.EnergyCalculator calculator,
    ...
    private EnergyAllocationPlan? CreateAllocationPlan(Models.EnergyCalculator calculator)
    {
    ...
    private static bool ShouldCreateNutritionAdvice(RuntimeArchive archive) =>
