...
本地时间转换只发生在最终 UI 渲染阶段。`EzNutrition.UI` 通过 `ILocalDateTimeFormatter` 获取宿主选择的显示时区；WASM 默认使用浏览器设备时区，未来 WPF 或机构宿主可以注入系统时区或机构配置时区。机器可读的 HTML `datetime` 属性仍使用 UTC，用户可见文本同时显示转换后的当地时间和偏移。
