...
`AuthenticationSessions` 记录会话归属、浏览器/桌面模式、固定安全戳、期限及撤销状态；`RefreshTokens` 记录摘要与消费时间。会话版本条件更新、消费旧凭据和写入新摘要位于同一数据库事务，多服务实例也只有一个请求能成功消费同一凭据。重复使用已消费凭据会撤销整个会话，包括它已经换出的 JWT。已消费记录保留到会话到期，由[每日维护清理任务](./maintenance-cleanup.md)在配置的服务器本地时间删除过期会话及关联记录。

每个受保护请求都校验 JWT、当前账号安全戳、锁定状态及会话是否有效。普通注销只撤销当前会话；改密、邮箱/手机号变更和现有权限管理操作沿用安全戳机制，使该账号全部旧会话失效；账号删除级联删除会话及刷新记录。

## 已知限制：旧账号的重复邮箱

2026-09-09 的角色声明修复只解耦权限变更与账号资料校验，没有修改登录策略。以下行为由合成 SQLite 账号和实际 Identity 验证器复现，见 `AuthManagerRepositoryLoginTests.DuplicateEmail.cs`：

- 登录按用户名 `FindByNameAsync` 定位账号，不按邮箱选择账号；共享邮箱不会自动合并身份或登录为另一个用户。
- 对邮箱已确认、未锁定且密码正确的重复邮箱账号：原失败计数为 0 时，密码校验通过，但写入 `LastSuccessfulLoginAtUtc` 调用 `UserManager.UpdateAsync`，被 `DuplicateEmail` 拒绝，抛出内部异常而不签发会话。HTTP 层按服务器异常处理，浏览器展示通用认证服务错误。
- 原失败计数非 0 时，Identity 重置失败计数也会触发同一校验，正确密码被作为登录失败处理，返回 401 的通用登录失败说明。
- 输入错误密码时，失败计数更新也会被拒绝；已验证数据库中的失败次数没有增加，不能依赖这类账号的累计失败锁定正常生效。现有请求限流不因此被移除。
- 已建立的会话不因重复邮箱本身自动失效；请求和刷新主要检查账号安全标记、锁定、确认状态及会话期限。角色权限变更会更新成员安全标记，因此可使旧会话失效，而邮箱冲突未处理的成员仍可能无法重新登录。

处理这批旧账号时，应先核实邮箱归属并规划账号恢复，再实际更新受影响角色的权限。不要通过取消注册/换邮箱的唯一性校验解决旧数据问题。是否让登录时间、失败计数更新与资料校验解耦，需要另行决定；当前文档记录现状，不宣称该登录限制已经修复。
...
`UserSessionService` 管理身份、短期令牌和会话状态，通过 `IAuthenticationSessionClient` 调用宿主。并发请求通过同一闸门共享刷新结果；单个请求取消只取消自己的等待。登录、退出或跨窗口变更会推进本地状态版本，迟到的旧响应不能恢复旧会话。

会话绝对期限固定，客户端可以据此结束本地身份；`RefreshExpiresAtUtc` 只是最近一次响应的空闲期限快照，其他窗口续期后可能过时。取令牌和生成界面认证状态都不能据此直接退出。需要刷新时，宿主在跨窗口锁内使用最新凭据，由服务端确认空闲期限和撤销状态；明确拒绝才清除对应身份，临时故障保留身份并阻止使用过期令牌发出业务请求。新响应仍校验期限字段，避免接受无效会话。

`PublicSystemInfoService` 通过匿名客户端独立加载备案、公告、协议和前后端版本信息，并发初始化共享一次加载。布局并行启动认证与公共信息初始化，页面分别订阅对应服务的通知；公共信息变慢或加载失败不会阻塞身份恢复和取令牌，登录退出也不会触发公共信息重新加载。
