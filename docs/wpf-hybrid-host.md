...
```text
Archives\
├── {document-id}.{content-sha256}.xml
├── {initial-report-version-id}.{content-sha256}.ezreport
└── .catalog\
    └── {document-id}.json
```

- 文档文件名由文档 GUID、正文指纹和格式扩展名组成，不含患者姓名。旧版仅使用 GUID 的文件名仍可读取。
- `.catalog` 保存调阅列表需要的格式无关摘要，包括标题、咨询对象显示文本、保存时间和格式信息；它被标记为隐藏目录，但没有加密。
- 单份文档上限为 16 MiB。读取在同一文件句柄上校验长度。保存先把正文写入同目录临时文件并刷新到磁盘，再生成以内容区分的文件，最后原子替换索引；旧正文直到索引提交后才清理。同一目录的写入使用文件锁，签发与更正使用预期旧正文作并发提交条件。
- 保存同一咨询时会替换对应草稿文档。应用允许删除普通咨询，拒绝删除正式报告；含正式报告时拒绝整库清空。允许清空时，宿主还会清理由中断写入留下、且文件名可被严格识别为宿主管理内容的孤立文档。
...
- **打开文档**使用 Windows 打开文件对话框读取 XML 或 `.ezreport`，校验后提供只读调阅。
- **导入报告包**使用同一文件选择器，校验 `.ezreport` 后保存到本机档案库。
- **导出档案**使用 Windows“另存为”对话框，默认从文档目录开始。成功写出后，资源管理器会打开并选中新文件；用户取消对话框会作为“已取消”返回，而不是误报成功。
- WPF 菜单中的 **文件 → 打开档案文件夹**用于检查、备份或迁移宿主管理目录。

建议备份或迁移时复制整个 `Archives` 目录，而不只复制 `.catalog` 或正文的其中一部分。外部导出的 XML 可以再次打开，但不会自动恢复为应用内已保存记录；`.ezreport` 可通过报告包导入入口恢复，包含各版 PDF 和签发历史。备份和迁移时应先完成当前保存操作。
