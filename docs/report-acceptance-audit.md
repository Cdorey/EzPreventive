...
能量报告验证（2026-09-10）：新增 `ReportWorkflowTests.Energy.cs`，覆盖配置确认/取消、节段全空拦截、独立权限、固定输入及原件、跨模块更正隔离、报告包导入重印，以及审核期间修改患者、PAL、能量、比例和交换份。全解决方案 Release 测试 760 项通过（Common 26、Contracts 79、XML 8、Application 82、Client 306、WPF 65、Server 194）；随后补充参考参数变化、手工核定和参考人群估算用例，报告工作流 74 项通过。最终全解决方案 Release 构建通过，0 警告、0 错误。
